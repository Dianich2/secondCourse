			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(17) << "Тип данных:" << std::setw(15) << "Значение:" << std::setw(27) << "Длина строки:" << std::setw(20) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::L)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].id << "\t\t\t\t\t" << "INT " << "\t\t\t" << this->table[i].value.vint << std::setw(30) << "-\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
			...
			*(idStream) << std::setw(25) << "Имя родительского блока:" << std::setw(20) << "Идентификатор:" << std::setw(16) << "Тип данных:" << std::setw(24) << "Тип идентификатора:" << std::setw(21) << "Первое вхождение:" << std::endl;
			...
			for (int i = 0; i < this->size; i++)
			{
				if (this->table[i].idtype == IT::IDTYPE::V)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(35 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(20) << "INT " << std::setw(22) << "V" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(35 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(20) << "STR " << std::setw(22) << "V" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
				...
				if (this->table[i].idtype == IT::IDTYPE::P)
				{
					...
					case 1:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(35 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(20) << "INT " << std::setw(22) << "P" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					case 2:
					{
						*(idStream) << "   " << this->table[i].parrent_function << std::setw(35 - strlen(this->table[i].parrent_function)) << this->table[i].id << std::setw(20) << "STR " << std::setw(22) << "P" << "\t\t\t" << this->table[i].firstApi << std::endl;
						break;
					}
					}
