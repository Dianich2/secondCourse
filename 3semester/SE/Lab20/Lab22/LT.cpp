		...
		if (lex_stream->is_open())
		{
			...
			for (int i = 0; i < this->size;)
			{
				...
				if (num_string == this->table[i].sn) {
					...
					/*if (this->table[i++].lexema == 'i' || this->table[i++].lexema == 'v') {
						(*lex_stream) << this->table[i++].lexema;
						(*lex_stream) << "(";
						(*lex_stream) << this->table[i++].idxTI;
						(*lex_stream) << ")";
					}
					else {
						(*lex_stream) << this->table[i++].lexema;
					}*/
					if (this->table[i - 1].lexema == 'i' || this->table[i - 1].lexema == 'v') {
						(*lex_stream) << "(";
						(*lex_stream) << this->table[i - 1].idxTI;
						(*lex_stream) << ")";
					}
