...
float startFeromones = 2, iterations = 3;
...
struct Ant {
    ...
    void PrintPath() {
        ...
        cout << "distance: " << antPath.distance << endl;
    }
};
...
struct GRAPH {
    ...
    const int SIZE;
    Path bestPath{ {},9999 };
    vector<vector<pair<float, float>>> gr;  // pair<COST, FEROMONE>
    ...
    void PrintGraph() {
        ...
        for (int i = 0; i < SIZE; ++i) {
            for (int j = 0; j < SIZE; ++j) {
                /*cout << setw(4) << fixed << setprecision(1) << gr[i][j].first << '/' << setprecision(2) << gr[i][j].second << " ";*/
    ...
    void UpdateFeromones() {
        for (auto& vec : gr) {
            for (auto& node : vec) {
                // cout << node.second << " : " << node.second*evaporation << endl;
...
vector<Step> calculatePossibilities(GRAPH& graph, Ant& ant) {
    ...
    for (auto node : ant.notVisited) {
        float posibility = pow(graph.gr[ant.currentPosition][node].second, alpha) * pow(graph.gr[ant.currentPosition][node].first, beta);
    ...
    for (auto node : ant.notVisited) {
        ...
        step.to = node;
        step.posibility = pow(graph.gr[ant.currentPosition][node].second, alpha) * pow(graph.gr[ant.currentPosition][node].first, beta) / posibilitySum;
...
int main(){
    ...
    cout << "Input start pheromons value : "; cin >> startFeromones;
    cout << "Input Alpha: "; cin >> alpha;
    cout << "Input Beta: "; cin >> beta;
    cout << "Input number of iterations: "; cin >> iterations;
    ...
    for (size_t iter = 0; iter < iterations; iter++) {  
        ...
        cout << "distance: " << graph.bestPath.distance << endl;
    }
